...
function openEnvelope() {
    ...
    // Create hearts animation (simultaneous with letter loading)
    createHeartAnimation();
    
    // Show first letter page immediately after hearts start
    setTimeout(() => {
        showLetterPage(1);
    }, 100);
}
...
function nextPage() {
    if (transitionLocked || currentPageIndex >= 4) return;
    currentPageIndex++;
    showLetterPage(currentPageIndex);
}

function prevPage() {
    if (transitionLocked || currentPageIndex <= 1) return;
    currentPageIndex--;
    showLetterPage(currentPageIndex);
}
...
function showLetterPage(pageIndex) {
    ...
    document.querySelectorAll('.photos-collage').forEach(collage => {
    ...
    if (pageIndex <= 3) {
        ...
        document.getElementById('letterPage4').classList.add('active');
    }
